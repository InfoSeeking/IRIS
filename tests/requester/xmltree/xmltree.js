...
(function($) {
	...
	XMLTree = function(jdo, subTreeRequest) {
		...
		------------------- */
		//ALBS I'm pretty sure these methods should not be added to the prototype, as every time a new xmlTree object is instantiated, the prototype is no longer valid for older instantiations
		this.actOnXML = function(xml) {
		...
		this.delve = function(node) {
			...
			//plus/mins indicator
			li.append($('<span>', {html: jdo.startExpanded ? '-' : '+'}).addClass('plusMin ' + (jdo.startExpanded ? 'expanded' : 'collapsed')));
			...
			if (!kids.length) {
				...
				if(jdo.shorten && nText.length > maxLength){
					extra = nText.substring(300);
					nText = nText.substring(0, 300) + "...";
					LITxtHolder.append(" <a href='#' class='expandExtra'>expand</a><div class='extra'>" + extra + "</div>");
				}
				LITxtHolder.prepend(nText).prepend(tagName);
			} else {
		...
		this.postBuild = function() {
			...
			this.tree.delegate(".expandExtra", "click", function(e){
				$(this).siblings(".extra").slideDown();
				$(this).removeClass().addClass("collapseExtra").html("collapse");
				e.preventDefault();
				e.stopPropagation();
				return false;
			});
			this.tree.delegate(".collapseExtra", "click", function(e){
				$(this).siblings(".extra").slideUp();
				$(this).removeClass().addClass("expandExtra").html("expand");
				e.preventDefault();
				e.stopPropagation();
				return false;
			});
